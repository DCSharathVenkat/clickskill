            ...
            <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
              ...
              <SheetContent side="right" className="w-[300px] sm:w-[400px]">
                <div className="flex flex-col space-y-4 mt-8">
                  <Link href="/about" className="block px-2 py-1 text-sm hover:bg-accent rounded font-semibold" onClick={() => setIsMobileMenuOpen(false)}>About</Link>
                  {[{ title: "Services", list: services }, { title: "Solutions", list: solution }, { title: "Industries", list: industries }, { title: "Resources", list: resources }].map((section) => (
                    <div className="space-y-2" key={section.title}>
                      ...
                      {section.list.map((item) => (
                        <Link key={item.name} href={item.href} className="block px-2 py-1 text-sm hover:bg-accent rounded" onClick={() => setIsMobileMenuOpen(false)}>{item.name}</Link>
                      ))}
                    </div>
                  ))}
                </div>
